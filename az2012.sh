...
echo "Update by Wyn @oatdonemdume"
echo "Repo: https://github.com/kmille36/Windows-11-VPS"
...
echo "Windows Server 2022 Datacenter" > abc; echo MicrosoftWindowsServer:WindowsServer:2012-Datacenter:latest > win 
...
image=$(cat win)
size=$(cat size)
rs=$(cat rs) && az vm create --resource-group $rs --name Windows-VM-PLUS --image $image --public-ip-sku Standard --size $size --location $location --admin-username administrator --admin-password WindowsPassword@001 --nic-delete-option delete --os-disk-delete-option delete --out table
